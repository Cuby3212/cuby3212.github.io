# 🌸 Cuby3212 (Página de enlaces)

Mi página de enlaces personal de código abierto.
Gracias a [2Duoble](https://twitter.com/2Duoble) por ayudarme con algunos ajustes de diseño de la página.

## ❓ ¿Qué es esta página?

Es una página simple con todos mis enlaces construida con LittleLink que he creado como alternativa a las típicas páginas de enlaces privativas que solía usar anteriormente (Ej. Linktree).

⛓️‍💥 **Enlace directo:** [cuby3212.github.io](https://cuby3212.github.io)

## 🖌️ ¿Qué se ha usado para crearla?

- [LittleLink](https://littlelink.io) — plantilla base en HTML/CSS
- Alojado en [GitHub Pages](https://pages.github.com/)

## 🗒️ Licencia

Basado en LittleLink, con licencia MIT. Ver [LICENSE.md](LICENSE.md).
